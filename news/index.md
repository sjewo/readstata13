...
- Initial support for Stata 18. Import .dtas files (Stata framesets) via
  [`read.dtas()`](../reference/read.dtas.md). Alias variables are
  currently ignored with a warning.
- The `select.cols` argument accepts either variable names or column
  indices.
...
- Fix compress option. In the past, unwanted conversions to integer type
  could occur.
...
- Working with strL variables is now a lot faster (thank to Magnus Thor
  Torfason)
...
CRAN release: 2015-03-24

- Fix heap overflow

## readstata13 0.6

CRAN release: 2015-03-22
...
- New replace.strl argument to replace the reference to a STRL string in
  the data.frame with the actual value
